...
@tiny function kernel_test!(RUx, RUy, Ux, Uy)
    ...
    if ix ∈ axes(RUx, 1) && iy ∈ axes(RUx, 2)
        @inbounds RUx[ix, iy] = Ux[ix, iy]^2 - 2.0f0 * Ux[ix+1, iy]^2 + Ux[ix+2, iy]^2 + 0.5f0 * Uy[ix+1, iy]^2
    end
    if ix ∈ axes(RUy, 1) && iy ∈ axes(RUy, 2)
        @inbounds RUy[ix, iy] = Uy[ix, iy]^2 - 2.0f0 * Uy[ix, iy+1]^2 + Uy[ix, iy+2]^2 + 0.5f0 * Ux[ix, iy+1]^2
    end
    return
end
...
function main(; device)
    nx, ny = 16, 16
    Ux  = device_array(Float32, device, nx, ny); copyto!(Ux, rand(nx, ny))
    Uy  = device_array(Float32, device, nx, ny); copyto!(Uy, rand(nx, ny))
    RUx = device_array(Float32, device, nx-2, ny); fill!(RUx, 0.0)
    RUy = device_array(Float32, device, nx, ny-2); fill!(RUy, 0.0)
    # AD VJP
    ∂Rx_∂R = device_array(Float32, device, nx-2, ny); fill!(∂Rx_∂R, 1.0)
    ∂Ry_∂R = device_array(Float32, device, nx, ny-2); fill!(∂Ry_∂R, 1.0)
